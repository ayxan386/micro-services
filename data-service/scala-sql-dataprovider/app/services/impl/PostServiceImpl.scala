...
@Singleton
class PostServiceImpl @Inject()(postRepository: PostRepository, userRepository: UserRepository, userService: UserService)(
  ...
  private def reqToModel(res: PostRequest) = Post(id = -1L, title = res.title, body = res.body, authorId = -1L)

  private def modelToResponse(post: Post) = PostResponse(title = post.title, body = post.body, author = None)
